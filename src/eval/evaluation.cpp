...
const int EndGame_Material = (1 * PIECE_VALUE[PIECE_WR]) + (2 * PIECE_VALUE[PIECE_WN]) + (2 * PIECE_VALUE[PIECE_WP]) + (PIECE_VALUE[PIECE_WK]);
...
int kingSafety(int kingSq, int color)
{
    ...
    if (pawnShieldMask)
    {
        ...
        while (missingPawn)
        {
            ...
            if ((pawnBitboard & bitboard->fileMasks[fileOf(sq)]) == 0)
            {
        ...
        while (pawnStorm)
        {
            ...
            score += pawnStormPenalty[color][rankOf(sq)];
...
int evalPosition()
{
    ...
    while (pieceBitboard)
    {
        ...
        // Brothers
        if (rookAttack & bitboard->fileMasks[fileOf(sq)] & otherRooks)
        {
            score += rookSupportSameFile;
        }
        if (rookAttack & bitboard->rankMasks[rankOf(sq)] & otherRooks)
        {
        ...
        if (((bitboard->pieces[PIECE_WP] | bitboard->pieces[PIECE_BP]) & bitboard->fileMasks[fileOf(sq)]) == 0)
        {
            score += RookOpenFile;
        }
        // Otherwise, if only the opponent's pawns remain on this file,
        else if ((bitboard->pieces[PIECE_WP] & bitboard->fileMasks[fileOf(sq)]) == 0)
        {
    ...
    while (pieceBitboard)
    {
        ...
        // Otherwise, if only the opponent's pawns remain on this file,
        else if ((bitboard->pieces[PIECE_BP] & bitboard->fileMasks[fileOf(sq)]) == 0)
        {
            score -= RookSemiOpenFile;
        }
    }
